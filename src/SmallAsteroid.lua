...
function SmallAsteroid:init()
    ...
    self.width = gTextures['meteor-small']:getWidth() / 4
    self.height = gTextures['meteor-small']:getHeight() / 4

    -- if picker1 is 1 then we spawn from the top
    if self.picker1 == 1 then
        self.x = math.random(0, VIRTUAL_WIDTH - self.width)
        self.y = -50
    else -- we spawn from the side
        -- left side spawn
...
function SmallAsteroid:collisions(target)
...
function SmallAsteroid:render()
    -- draw the asteroid at 25% size (idk why I called them meteor in dependencies)
    love.graphics.draw(gTextures['meteor-small'], self.x, self.y, 0, 0.25, 0.25)
end